## [AI 개념](https://docs.spring.io/spring-ai/reference/1.0/concepts.html)

### Model

- 정보를 처리하고 생성하도록 설계된 알고리즘.
- 대규모 데이터 세트에서 패턴과 통찰력을 학습하여 예측, 텍스트, 이미지 또는 기타 결과물을 생성
- 모델 종류: LLM(Large `Language` Model), `Image` Generation Model, Text-`Voice` Model, `Embedding` Model, GPT

LLM 종류

- GPT - openai (멀티모달모델: 텍스트,이미지,비디오,음성)
- 제미나이 - 구글
- 라마 - 메타
- 클로드(하이쿠, 소네트, 오퍼스) - 앤트로픽
- 딥시크 - 중국

LLM 을 활용한 생성형 AI 서비스

- 쳇GPT
- 퍼플렉시티: 인공지능을 기반으로 하는 검색엔진
- 에이닷: SKT. 문서요약 -> 회의록정리 -> 참석자 메일발송
- 인프런 AI 인턴

### 프롬프트

- AI 모델이 특정 출력을 생성하도록 안내하는 언어 기반 입력
- 프롬프트 텍스트는 단순한 문자열이 아니고 프롬프트 내에 여러 개의 텍스트 입력 필드를 가지고 있으며, 각 입력 필드에는 역할이 할당됩니다. 예를 들어, `시스템 역할`은 모델의 동작 방식을 알려주고 상호작용의 맥락을 설정합니다. `사용자 역할`은 일반적으로 사용자의 입력 필드입니다.

### 프롬프트 템플리

- 프롬프트 생성 및 관리를 위해 기존의 텍스트 기반 템플릿 엔진을 사용
- [StringTemplate](https://www.stringtemplate.org/) 오픈소스 라이브러리 활용
- AI 모델에게 전달할 복잡한 지시문(Prompt)을 코드와 분리하여 체계적으로 관리
- 구조화된 프롬프트: <system_instruction>, <user_input>, <context> 등을 변수로 비워두고, 실행 시점에 StringTemplate을 이용해 안전하게 데이터를 채워 넣습니다.
- AI가 단순히 채팅만 하는 것이 아니라 코드를 생성하거나 특정 형식(JSON, SQL 등)의 결과물을 만들어야 할 때 ST가 강력한 힘을 발휘
- RAG 컨텍스트 주입: 벡터 데이터베이스에서 검색해 온 방대한 참고 자료(Context)를 프롬프트의 특정 위치에 깔끔하게 배치하는 역할을 수행합니다

### 임베딩

- 임베딩은 텍스트, 이미지 또는 비디오의 수치적 표현으로, 입력 요소 간의 관계를 나타냅니다.
- 임베딩은 텍스트, 이미지, 비디오를 `벡터`라고 하는 부동 소수점 숫자 `배열`로 변환하는 방식으로 작동
- 애플리케이션은 두 텍스트의 벡터 표현 간의 수치적 거리를 계산함으로써 임베딩 벡터를 생성하는 데 사용된 `객체 간의 유사성`을 판단할 수 있습니다.
- 임베딩은 검색 증강 생성(`RAG`) 패턴과 같은 실제 응용 분야에서 특히 중요

<img src="images/openai10.png" width="1000">

### 토큰

- 토큰은 AI 모델 작동 방식의 기본 구성 요소입니다. 모델은 입력을 받으면 단어를 토큰으로 변환하고, 출력할 때는 토큰을 다시 단어로 변환합니다.
- 영어에서 토큰 하나는 단어의 약 75%에 해당합니다. 참고로, 셰익스피어의 전집은 약 90만 단어로 구성되어 있으며, 이는 약 120만 개의 토큰으로 환산됩니다.
- 모델은 토큰 제한을 받는데, 이는 단일 API 호출에서 처리할 수 있는 텍스트 양을 제한합니다. 이 제한을 흔히 "컨텍스트 윈도우"라고 합니다. 모델은 이 제한을 초과하는 텍스트는 처리하지 않습니다.

<img src="images/openai11.png" width="600">

### 구조화된 출력

### 데이터와 API를 AI 모델에 활용하기

인공지능 모델에 학습되지 않은 정보를 어떻게 제공할 수 있을까요?

- 미세조정 : 전통적인 머신러닝 기법은 모델을 맞춤 설정하고 내부 가중치를 변경
- 프롬프트 스터핑 -> RAG를 기반으로 하는 솔루션
- 툴 호출 : LLM을 오부 시스템의 API에 연결하는 도구를 등록할 수 있습니다.

### RAG(Retrieval Augmented Generation)

이 접근 방식은 배치 처리 방식의 프로그래밍 모델을 사용하는데, 작업이 문서에서 비정형 데이터를 읽어 변환한 다음 벡터 데이터베이스에 기록합니다. 간단히 말하면, 이는 ETL(추출, 변환, 로드) 파이프라인입니다. 벡터 데이터베이스는 RAG 기법의 검색 부분에서 사용됩니다.

<img src="images/openai12.png" width="1100">
...
- 프롬프트 엔지니어링: 언어모델의 답변을 최적화하기 위해 입력프롬프트를 설계하는 것
- 파인튜닝: 이미 학습된 언어 모델에 원하는 분야나 특정 용도에 맞게 추가 데이터를 학습시키는 기법
- RAG: LLM은 학습을 완료한 시점 이후 생성된 외부의 정보는 알지 못합니다. 필요한 정보를 검색해서 답변할 때 활용하도록 돕는 기술
- 평션콜링과 도구호출: 외부 API나 직접만든 함수를 호출하여 그 결과를 바탕으로 답변
- 랭체인: LLM을 활용하여 애플리케이션을 개발하는 프레임워크
- 랭그래프: 여러 AI 에이전트를 만들어 협업하도록 시스템을 구성하는 방식을 멀티 에이전트라고 하고 이를 구현하는 방법중에 하나임.
...
- [spring ai 실습](https://github.com/spring-ai-community/awesome-spring-ai)
- [playground-flight-booking](https://github.com/tzolov/playground-flight-booking)
...
Allama는 로컬 환경에서 LLM을 쉽게 실행할 수 있게 해주는 오픈소스 도구
