...
| @Autowired |                                                                               |
| @Qualifier |                                                                               |
| @Primary   | 클래스타입이 같은 여러 스프링 빈 중에서 @Primary 설정된 스프링 빈이 우선 주입 |

stereotype 어노테이션  
클래스의 목적, 디자인 패턴에 따라 적절한 어노테이션을 선택해서 사용
| 어노테이션 | 설명 |
| :--------- | :---------------------------------------------------------------------------- |
| @Component | |
| @Controller |컨트롤러 역할을 하는 클래스를 스프링 빈으로 정의 |
| @Service | 도메인 주도 설계(Domain Driven Development) 의 서비스 역할인 클래스에 정의. 여러 객체에서 일어나는 행위를 하나의 기능으로 모아서 처리하는 클래스 |
| @Mapper | |
| @Repository | DDD의 리포지토리를 의미. 객체를 저장, 조회하는 행위를 담당하는 클래슨 |
